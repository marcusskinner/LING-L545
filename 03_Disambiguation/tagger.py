...
class PerceptronTagger():
	...
	def tag(self, corpus, flags, tokenise=False):
...
#				print('s:',sentence)
				for words in sentence:	
					...
					for i, token in enumerate(sentence):
						...
						if not tag:
							...
							features = self._get_features(i, token[1], context, prev, prev2, flags)
							# make the prediction
	...
	def train(self, sentences, flags, save_loc=None, nr_iter=5):
...
#				print(c, n, '|||', sentence);
				...
				for i, token in enumerate(sentence):
					...
					if not guess:
						feats = self._get_features(i, word, context, prev, prev2, flags)
	...
	def _get_features(self, i, word, context, prev, prev2, flags):
...
def tagger(corpus_file, model_file, flags):
	''' tag some text. 
	...
	t.tag(corpus_file, flags)

def trainer(corpus_file, model_file, flags):
	''' train a model 
	:param corpus_file is a file handle
	:param model_file is a saved model file
	'''
	...
	t.train(sentences, flags, save_loc=model_file, nr_iter=5)

# Main function

if sys.argv[1] == '-t':
	trainer(sys.stdin, sys.argv[2], sys.argv[3:])
elif len(sys.argv) == 2:
	tagger(sys.stdin, sys.argv[1], sys.argv[2:])
else:
